...
class ExperimentPlannerTransUNet3D(ExperimentPlanner):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        super(ExperimentPlannerTransUNet3D, self).__init__(folder_with_cropped_data, preprocessed_output_folder)
        self.data_identifier = "nnUNetData_plans_transUNet3D"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnUNetPlans_transUNet3D_plans_3D.pkl")
    ...
    def get_properties_for_stage(self, current_spacing, original_spacing, original_shape, num_cases,
        ...
        # hard-coded
        input_patch_size = np.ceil(new_median_shape/2/16).astype(np.int)*16

        t = torch.cuda.get_device_properties(0).total_memory
        batch_size = self.unet_min_batch_size
        while GenericTransUNet3D.use_this_for_batch_size_computation_3D /2 * (batch_size + 2) < t:
            batch_size += 2
    ...
    def plan_experiment(self):
        ...
        config = get_r50_b16_3d_config(len(all_classes) + 1)
